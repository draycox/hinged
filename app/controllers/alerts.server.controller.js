 ...
 * List of Alerts
 */
exports.list = function(req, res) { Alert.find().sort('-created').populate('user', 'displayName').exec(function(err, alerts) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 * Alert middleware
 */
exports.alertByID = function(req, res, next, id) { Alert.findById(id).populate('user', 'displayName').exec(function(err, alert) {
