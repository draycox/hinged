 ...
 * List of Rewards
 */
exports.list = function(req, res) { Reward.find({ user: req.user.id}).sort('-created').populate('user', 'displayName').exec(function(err, rewards) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 * Reward middleware
 */
exports.rewardByID = function(req, res, next, id) { Reward.findById(id).populate('user', 'displayName').exec(function(err, reward) {
